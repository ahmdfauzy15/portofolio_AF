import React from 'react';
import { motion } from 'framer-motion';
import { styles } from '../style';
import Typewriter from 'react-typewriter-effect';
import computerImg from '../assets/computers.jpg'; // Import gambar dari src/assets

const Hero = () => {
  return (
    <section className='relative w-full h-screen mx-auto flex flex-col justify-center items-center'>


      {/* Teks & Animasi */}
      <div className={`${styles.paddingX} absolute inset-0 top-[120px] max-w-7xl mx-auto flex flex-row items-start gap-5`}>
        <div className='flex flex-col justify-center items-center mt-5'>
          <div className='w-5 h-5 rounded-full bg-[#915eff]' />
          <div className='w-1 sm:h-80 h-40 violet-gradient' />
        </div>
        <div>
          {/* Teks Gradasi Ala Alam Semesta */}
          <h1 className={`${styles.heroHeadText} bg-gradient-to-r from-blue-400 via-purple-500 to-indigo-600 text-transparent bg-clip-text`}>
            Hi, I'm Fauzy<span className='font-bold drop-shadow-lg'></span>
          </h1>
          <p className={`${styles.heroSubText} mt-2 text-white-100`}>
            Web & Android Developer
          </p>
        </div>
      </div>


      {/* <motion.div
      
        className="absolute inset-x-0 bottom-20 flex justify-center w-full" // Naikkan posisi gambar
        initial={{ y: 20, opacity: 0.8 }}
        animate={{ y: [20, 0, 20], opacity: 1 }}
        transition={{ duration: 3, repeat: Infinity, repeatType: "reverse" }}
      >
        <img 
          src={computerImg} 
          alt="Computer" 
          className="w-[400px] md:w-[500px] lg:w-[500px] object-contain drop-shadow-2xl rounded-xl"
        />
      </motion.div> */}

      <div className='absolute xs:-bottom-10 z-10 -bottom-10 w-full flex justify-center items-center'>
        <a href="#about">
          <div className='w-[35px] h-[64px] rounded-3xl border-4 border-secondary flex justify-center items-start p-2'>
            <motion.div
              animate={{ y: [0, 24, 0] }}
              transition={{ duration: 2, repeat: Infinity, repeatType: 'loop' }}
              className='w-3 h-3 rounded-full bg-secondary mb-1'
            />
          </div>
        </a>
      </div>
    </section>
  );
};

export default Hero;
